...
import { EntityId, IDataResponse, IPaginatedResponse } from '@/services/core'
import type {
    AuthLoginRequest,
    AuthLoginResponse,
    AuthRefreshRequest,
...
export const loginAuditService = {
    ...
    list(query?: ILoginAuditFilters): Promise<IPaginatedResponse<ILoginAuditItem>> {
        return apiClient.get<IPaginatedResponse<ILoginAuditItem>>(API_ENDPOINTS.loginAudit.root, {
            query: query ? { ...query } : undefined,
        })
    },

    getStats(query?: ILoginAuditFilters): Promise<IDataResponse<ILoginAuditStats>> {
        return apiClient.get<IDataResponse<ILoginAuditStats>>(API_ENDPOINTS.loginAudit.stats, {
            query: query ? { ...query } : undefined,
        })
    },
